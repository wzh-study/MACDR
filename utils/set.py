...
def save_checkpoint(model, model_save_path, best_ckpt, max_checkpoints=1):
    # 保存当前模型文件
    torch.save(model.state_dict(), model_save_path + best_ckpt)
    
    # 获取所有模型文件并按时间排序
    checkpoint_files = glob.glob(os.path.join(model_save_path, "*.ckpt"))

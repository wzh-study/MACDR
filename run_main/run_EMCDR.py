...
class Run(Datasets):
    def __init__(self, 
        ...
        record_path = './saved/' + str(int(self.ratio[0] * 10)) + '_' + str(int(self.ratio[1] * 10)) + '/tgt_' + self.tgt + '_src_' + self.src + '_results/EMCDR/'+'runid_'+str(self.runid)+'/'
    ...
    def main(self):
        model = EMCDR(self.config)  #  (181187, 114495, 2, 10, 50)
        if len(os.listdir(self.model_save_path)) > 0:
            file_names = os.listdir(self.model_save_path)
            checkpoint = torch.load(self.model_save_pre_train_path + file_names[0])
            model.load_state_dict(checkpoint)


        model.src_model.user_embeddings  
        model.tgt_model.user_embeddings
